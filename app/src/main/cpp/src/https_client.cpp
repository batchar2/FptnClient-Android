 ...
 * @param[in] md5_fingerprint_param Expected server certificate fingerprint as Java string
 *
    ...
    jint timeout_param)
{
    ...
    if (https_client) {
        ...
        env->SetObjectArrayElement(result, 1, env->NewObject(
                env->FindClass("java/lang/Integer"),
                env->GetMethodID(env->FindClass("java/lang/Integer"), "<init>", "(I)V"),
                resp.code
        ));
        env->SetObjectArrayElement(result, 2, env->NewStringUTF(resp.errmsg.c_str()));
    } else {
        // default value
...
/**
 * @brief Makes an HTTP POST request and returns the response as a Java Object array.
 *
 ...
 * @param[in] http_handle_param URL endpoint for the POST request (as Java String)
